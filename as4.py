...
def answer_one():
    excel = pd.ExcelFile('Energy Indicators.xls')
    energy = excel.parse(skiprows=17,skip_footer=(38))
...
def answer_three():
    Top15 = answer_one()
    GDPavg = Top15[['2006','2007','2008','2009','2010','2011','2012','2013','2014','2015']].mean(axis = 1).rename('avgGDP').sort_values(ascending= True)
    return pd.Series(GDPavg)
answer_three()
...
def answer_six():
    Top15 = answer_one()
    find_mx = Top15.iloc[:,9].max()
    return ('Brazil',find_mx)
answer_six()
...
def answer_eight():
    ...
    mx = sorted(Top15['Pop'],reverse = True)[2]
    a = str(pd.Series(Top15[Top15['Pop']==mx].index)).split()
...
def answer_ten():
    Top15 = answer_one()
    f_med = Top15.iloc[:,9].median()
    Top15['HighRenew']  = None
    for i in range(len(Top15)):
        if Top15.iloc[i,9] > f_med:
            Top15.iloc[i,20] = 1
        else:
            Top15.iloc[i,20] = 0
